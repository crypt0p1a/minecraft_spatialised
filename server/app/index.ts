...
interface MapUUID {
  [uuid: string]: { x: number, y:number, z: number, yaw: number, pitch: number, scale: number}
}
      ...
      switch(object.type||"") {
        ...
        case "PositionUpdate": {
          ...
          uuids.forEach(uuid => {
            ...
            const { x, y, z, yaw, pitch, scale } = position;

            this.io?.emit("position", uuid, x, y, z, yaw, pitch, scale);
          });
          break;
        }
        default:
